...
- 78 implemented mapper IDs; seven board families currently have reproducible external or pinned
...
(Sachen SA-020A) and 245 (Waixing F003) were added afterwards from current hardware documentation and
corpus evidence. Those post-track additions do not expand scope to every assigned iNES/NES 2.0
number or permit title hashes and guessed board variants. External `Verified` evidence remains a
separate follow-up.
...
242, 243, 245 and 248 are implemented but do not yet all have executable external verification.
