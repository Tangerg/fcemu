...
/** Resolves and validates the two unrelated hardware families assigned mapper 34. */
export function resolveMapper34Board(cartridge: Cartridge): Mapper34BoardKind {
  const kind = resolveKind(cartridge);
  if (kind === "nina-001") validateNina001(cartridge);
  else validateBnrom(cartridge);
  return kind;
}

function resolveKind(cartridge: Cartridge): Mapper34BoardKind {
...
function validateNina001(cartridge: Cartridge): void {
  if (![0x10_000, 0x20_000].includes(cartridge.prgRom.byteLength)) {
    throw configurationError(cartridge, "NINA-001 requires 64 or 128 KiB of PRG ROM");
  }
  if (
    cartridge.hasWritableChrMemory ||
    ![0x2000, 0x4000, 0x8000, 0x10_000].includes(cartridge.chrRom.byteLength)
  ) {
    throw configurationError(cartridge, "NINA-001 requires 8, 16, 32 or 64 KiB of CHR ROM");
  }
  if (cartridge.prgWritableBytes !== 0x2000) {
    throw configurationError(cartridge, "NINA-001 requires its unbanked 8 KiB PRG RAM");
  }
  requireDirectPrgRam(cartridge);
}

function validateBnrom(cartridge: Cartridge): void {
  if (![0x8000, 0x10_000, 0x20_000].includes(cartridge.prgRom.byteLength)) {
    throw configurationError(cartridge, "BNROM requires 32, 64 or 128 KiB of PRG ROM");
  }
  if (cartridge.chrMemoryBytes !== 0x2000) {
    throw configurationError(cartridge, "BNROM requires 8 KiB of unbanked CHR memory");
  }
  requireDirectPrgRam(cartridge);
}
...
function requireDirectPrgRam(cartridge: Cartridge): void {
  ...
  if (cartridge.prgRamBytes > 0 && cartridge.prgNvRamBytes > 0) {
    throw configurationError(cartridge, "mixed PRG RAM/NVRAM is not supported by this board");
  }
}
