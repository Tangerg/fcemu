...
describe("Mapper 34 board identity", () => {
  it("uses CHR geometry to select exactly one legacy board", () => {
    ...
    expect(resolveMapper34Board(bnrom)).toBe("bnrom");
    expect(resolveMapper34Board(nina001)).toBe("nina-001");
  });
  ...
  it.each([
    ...
    expect(resolveMapper34Board(cartridge)).toBe(kind);
  });
  ...
  it.each([
    ...
    expect(() => resolveMapper34Board(cartridge)).toThrow(
