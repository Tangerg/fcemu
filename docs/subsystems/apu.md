...
travels inside
the console's current version 19 save-state envelope. Output-filter history first entered the schema
...
MMC3 IRQ output and named mapper IRQ-source identity, version 18 added Oeka Kids tablet state, and
version 19 separated exact K-1029 state from legacy mapper-15 compatibility state.
